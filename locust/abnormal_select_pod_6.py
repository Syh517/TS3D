        ...
        with open("/home/yyy/mysql/data/logs/logs.txt",'a') as file:
            try:
                conn = pymysql.connect(
                    ...
                    port=int(self.get_config("DB_PORT", 32346)),
                ...
                now = datetime.now()
                log="数据库连接成功"
                file.write(f"[{now}][{self.node}][database_abnormal][abnormal_select_pod_6][info][{log}]\n")
                logger.info("数据库连接成功")
                return conn
            except MySQLError as e:
                now = datetime.now()
                log="数据库连接失败"
                file.write(f"[{now}][{self.node}][database_abnormal][abnormal_select_pod_6][error][{log}][{e}]\n")
    ...
    def select_1(self):
        with open("/home/yyy/mysql/data/logs/logs.txt",'a') as file, open("/home/yyy/mysql/data/logs/sqls.txt",'a') as file2:
            ...
            if self.db_conn is None:
                now = datetime.now()
                log="数据库连接未初始化"
                file.write(f"[{now}][{self.node}][database_abnormal][abnormal_select_pod_6][error][{log}]\n")
            ...
            try:
                with self.db_conn.cursor() as cursor:
                    ...
                    if result:
                        ...
                        file2.write(f"{now}\t{self.node}\tabnormal_select_pod_6\t'{sql}'\t1\n")
                    else:
                        now = datetime.now()
            ...
            try:
                with self.db_conn.cursor() as cursor:
                    ...
                    if result:
                        ...
                        file.write(f"[{now}][{self.node}][database_abnormal][abnormal_select_pod_6][info][{log}]\n")
                        logger.info(f"{function_name}数据查询成功")
                        file2.write(f"{now}\t{self.node}\tabnormal_select_pod_6\t'{sql0}'\t1\n")
                    else:
                        now = datetime.now()
                        log=f"未查询到{function_name}数据"
                        file.write(f"[{now}][{self.node}][database_abnormal][abnormal_select_pod_6][info][{log}]\n")
